...
class Interface:
    ...
    def __init__(self, ifname, time=1, unit=KB):
        ...
        self.ifname_rx_packets = os.path.join(
            ifname_dir, ifname, ifname_rx_packets)
        self.ifname_tx_packets = os.path.join(
            ifname_dir, ifname, ifname_tx_packets)
...
def main():
    ...
    parse = argparse.ArgumentParser(
        usage='Usage: %(prog)s [-t <1>] [ -i <iface> ] [-k|-m|-g] ')
    # add_help=True)
    ...
    while True:
        clear()
        print('{:<16}{:>12}{:>12}{:>12}{:>12}{:>12}{:>12}'.format('iface', 'Rxpck/s',
                                                                  'Txpck/s', 'Rx{}/s'.format(UNIT), 'Tx{}/s'.format(UNIT), 'Rxstatis', 'Txstatis'))
        for ifs in ifnames:
            try:
                ifs.speed()
