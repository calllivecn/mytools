...
class EmailSender:
    ...
    def __init__(self, server: str, port: int = 465, email: list[str] = '', password: str = '', verbose=False):
        self.server = server
        self.port = port
        self.email = email
    ...
    def send(self, to_email: list[str]):
...
def main():
    ...
    es = EmailSender(server, port, email, password, args.verbose)
