...
class Sql3():
	...
	def add(self,db_lists):
		self.db.executemany('insert into sha512(filename,filepath,filesize,sha512) values(?,?,?,?)',db_lists)
		self.db.commit()

	def __update_sha(self,db_lists):
		self.db.executemany('update sha512 set sha512=? where filepath=?;',db_lists)
		self.db.commit()
	...
	'''
		...
		for f in multi_file: #.fetchall():
			fetch = self.db.execute('select * from sha512 where filesize=?;',f)
